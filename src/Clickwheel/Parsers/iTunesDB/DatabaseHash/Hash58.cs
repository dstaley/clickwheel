...
namespace Clickwheel.DatabaseHash
{
    ...
    internal static class Hash58
    {
        ...
        public static byte[] GenerateHash(byte[] fwid, byte[] iTunesDB)
        {
            ...
            using var dbHash = SHA1.Create();
            dbHash.TransformBlock(key, 0, 64, null, 0);
            dbHash.TransformFinalBlock(iTunesDB, 0, iTunesDB.Length);
            hash = dbHash.Hash;
            ...
            using var dbHashHash = SHA1.Create();
            dbHashHash.TransformBlock(key, 0, 64, null, 0);
            dbHashHash.TransformFinalBlock(hash, 0, 20);
            return dbHashHash.Hash;
        }
        ...
        private static void GenerateKey(byte[] fwid, byte[] key)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                var lcm = Lcm(fwid[i * 2], fwid[i * 2 + 1]);
                var hi = (byte)((lcm & 0xFF00) >> 8);
                var lo = (byte)(lcm & 0xFF);
